...
内部バッファを利用してInputStreamからの入力を効率的に処理し、通常の`Scanner`クラスよりも高速な処理が可能です。
`FastIO24`では、Java 24の最新機能を活用して最適化されています。以前の `ContestScanner` の機能（配列やコレクションの一括読み込みなど）も統合されています。

## 特徴

- **効率的な入力**: 内部バッファを利用して効率的に入力処理を行います。
- **多機能**: 1次元から3次元までの配列、ソート済み配列、累積和配列、各種コレクション、マルチセットなど、多彩な入力パターンを一行で記述できます。
- **高速性の重視**: 数値パースロジックを最適化するなど、パフォーマンスを重視しています。
...
| **バージョン 4.0** | 2026-02-15 | `FastIO24`において `ContestScanner` 機能を統合。すべての拡張入力メソッドが `FastScanner` で利用可能になりました。                       |
