...
- **堅牢なリソース管理**: `close()` メソッドが強化され、入力ストリームの確実なクローズと内部状態のリセットを行います。
...
- `FastScanner`の数値パースロジックが `do-while` ループに最適化され、より効率的な数値読み込みが可能です。
...
| **バージョン 3.0** | 2025-11-11 | `FastIO24`としてJava 24向けに最適化。`FastScanner`の内部ロジック変更に伴う更新。                                               |
