...
`FastScanner` / `FastPrinter`（src 本体）と、比較用実装 `FastScanner2` / `FastPrinter2`（ストリーミング型・本フォルダ内）の性能を比較計測するためのベンチマーク一式です。
...
"./Data Structures/FastIO24/Benchmark/run_tests_24.sh" [RUN_COUNT] [TOKEN_COUNT] [STRING_PRINT_COUNT]
```

```bat
:: Windows
"Data Structures\FastIO24\Benchmark\run_tests_24.bat" [RUN_COUNT] [TOKEN_COUNT] [STRING_PRINT_COUNT]
```
