...
| `sumRange(int l, int r)`       | `long` | インデックスlからrまでの区間和を返します      |
| `uniqueSumRange(int l, int r)` | `long` | ユニークインデックスlからrまでの区間和を返します  |
...
System.out.println(multiSet.sumRange(1, 3)); // 40 (10+10+20)
...
- `first()`, `last()`, `pollFirst()` 等は空のマルチセットで呼ぶと例外をスローします
- インデックスは0から始まります
- スレッドセーフではありません
