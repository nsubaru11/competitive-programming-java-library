...
/**
 * int値の2次元累積和を一次元配列で保持します。
 */
public final class IntPrefixSum2D {
	 ...
	 */
	public IntPrefixSum2D(final int h, final int w, final IntBinaryOperator init) {
		this.h = h;
		this.w = w;
		length = h * w;
		stride = w + 1;
		...
		for (int i = 0; i < h; i++) {
			int p = (i + 1) * stride + 1;
			for (int j = 0; j < w; j++, p++) {
				pref[p] = init.applyAsInt(i, j) + pref[p - stride] + pref[p - 1] - pref[p - stride - 1];
			}
		}
	}
	 ...
	 * 指定された配列から構築します。
	 */
	public IntPrefixSum2D(final int[][] a) {
		h = a.length;
		w = a[0].length;
		length = h * w;
		stride = w + 1;
	 ...
	 */
	public static IntPrefixSum2D generate(final int h, final int w, final IntSupplier init) {
		return new IntPrefixSum2D(h, w, (_, _) -> init.getAsInt());
	}
