...
public final class IntArrayDeque implements IntMutableArray, Cloneable {
	...
	/**
	 * 指定された初期容量でIntArrayDequeを構築します。
	 *
	 * @param capacity 初期容量。内部で2のべき乗に正規化されます。
	 */
	public IntArrayDeque(final int capacity) {
	 ...
	 * @throws IndexOutOfBoundsException インデックスが範囲外の場合
	 */
	public int set(int index, final int e) {
	...
	public IntArrayDeque clone() {
		try {
			IntArrayDeque rb = (IntArrayDeque) super.clone();
	...
	/**
	 * 要素と順序が一致するIntArrayDequeかを判定します。
	 *
	 * @param o 比較対象のオブジェクト
	...
	public boolean equals(Object o) {
		...
		if (!(o instanceof IntArrayDeque other)) return false;
	...
	public String toString() {
		if (isEmpty()) return "";
		final StringBuilder sb = new StringBuilder(12 * size - 1);
		sb.append(buf[head]);
