...
- `convolveAnd` / `convolveOr`は、部分集合・上位集合ゼータ変換を用いて利用できます。`convolveXor`はFWHTを用いて利用できます。
- 通常の多項式畳み込みは、内部の変換長にかかわらず`a.length + b.length - 1`要素を返します。
- NTT、FFT、GCD・LCM畳み込み、およびGarner復元は未実装です。
...
- FWHT、部分集合・上位集合ゼータ変換とメビウス変換は利用可能です。
- NTT、FFT、約数・倍数ゼータ変換とメビウス変換は未実装です。
