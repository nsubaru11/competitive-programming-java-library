...
剰余版では係数を`[0, mod)`に正規化して渡してください。NTTでは`mod`が素数であり、変換長が`mod - 1`を割り切ることを前提とします。FWHTによる逆変換には変換長と`mod`が互いに素であることが必要です。`long`版は通常の競技プログラミングで使う範囲のmodと係数を対象とし、積・和が`long`に収まることを前提とします。

## 変換API

| 変換群       | コピー版                             | in-place版                                      | 状態     |
|--------------|--------------------------------------|-------------------------------------------------|----------|
| NTT          | `ntt`                                | `nttInPlace`                                    | 利用可能 |
...
`convolveNtt`は内部でビット反転順のバタフライ変換を正変換から逆変換まで維持し、公開`ntt`で保証する通常順への並べ替えを省略します。回転因子は同じブロック内で共有し、法と原始根から一度作成した更新率表を2回の正変換と逆変換で再利用します。
