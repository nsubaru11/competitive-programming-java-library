...
倍数ゼータ変換は`F[d - 1] = Σ_{d | i} a[i - 1]`、約数ゼータ変換は`F[m - 1] = Σ_{d | m} a[d - 1]`を計算します。対応するメビウス変換はそれぞれの逆変換です。素数ごとに変換を適用するため、時間計算量は篩を含めて`O(N log log N)`です。既存の[`PrimeTable`](../PrimeTable)を最大要求長までキャッシュし、同じ長さ以下の後続変換で再利用します。
