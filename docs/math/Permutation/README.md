...
順列（要素の並び替え）を効率的に生成・解析するためのアルゴリズムを提供します。
辞書順での現在位置と、次の順列・前の順列を扱い、様々なデータ型に対応しています。

## 実装クラス

### [Permutation](../../../src/lib/math/Permutation.java)

- **用途**：
	- 辞書順での順列生成
- **特徴**：
	- 辞書順で**次の順列** (`next`) および**前の順列** (`prev`) を生成します。
	- 重複する並びを除いた辞書順で、現在の順列の0始まり位置 (`index`) を求めます。
