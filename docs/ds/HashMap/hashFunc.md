...
現行のMap実装はミキサー適用前に `ThreadLocalRandom` で生成したsaltをキーへXORします。
`TestHash` は方式間の比較を再現可能にするため、全方式へ同じ固定saltを適用します。
以下のコード例は、比較対象となるミキサー本体だけを示しています。

| 方式名                        | 全体時間 (1回目 / 2回目) | 平均時間 (1回目 / 2回目) | 評価・特徴                                                   |
|:------------------------------|:-------------------------|:-------------------------|:-------------------------------------------------------------|
| **MurmurHash3系軽量ミキサー** | **159ms** / 172ms        | **101.3ms** / 105.3ms    | **最速・最安定。** 撹拌と計算コストのベストバランス。        |
...
64ビット版と同様に、現行のMap実装はランダムsalt、`TestHash` は固定saltをミキサー適用前に使用します。以下のコード例では共通のsalt処理を省略しています。
