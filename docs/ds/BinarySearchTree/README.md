...
### [BinarySearchTree](../../../src/lib/ds/BinarySearchTree.java)
...
- **時間計算量**：検索/挿入/削除/順位検索 期待 O(log n)
- **空間計算量**：O(n)

### [IntTreap](../../../src/lib/ds/IntTreap.java)（一部未実装）

- **用途**：int特化版のTreap（ボクシング回避による高速化）
- **注意**：`split` 系メソッドは未実装（呼び出すと例外）、`merge` はキー範囲が交差する場合に未対応（TODO）

### [VanEmdeBoasTree](../../../src/lib/ds/VanEmdeBoasTree.java) (計画中)

- **用途**：整数キー集合に対する挿入・削除・前後検索を O(log log U) で提供
- **特徴**：実装予定（TODO）

### [RedBlackTree](../../../src/lib/ds/RedBlackTree.java) (計画中)
...
### [BTree](../../../src/lib/ds/BTree.java) (計画中)
...
- **Treap / IntTreap**: 順序統計付きの平衡BSTが必要な場合に適しています。最悪ケースでも期待 O(log n) を維持します。int キーなら IntTreap が高速です。
