...
`Trie` は文字列集合に対する前方一致（プレフィックス）操作を高速に行うための木構造です。 本実装は競技プログラミング向けの最小機能版で、単語の挿入・完全一致検索・プレフィックスに一致する単語数の取得を $\mathcal{O}(L)$ 時間（L は文字列長）で行えます。

## 特徴

- ハッシュマップベースの素直な実装（子ノードは `HashMap<Character, TrieNode>`）
- ルート直下に子ノードを持つシンプル設計（`lib.ds`パッケージ、外部依存なし）
