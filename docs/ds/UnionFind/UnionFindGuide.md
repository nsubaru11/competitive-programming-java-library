...
- `groups()`は$\mathcal{O}(n)$で全要素走査します

## パフォーマンス特性

- 初期化：$\mathcal{O}(n)$
- find/union/isConnected：償却$\mathcal{O}(\alpha(n))$（α (n)はアッカーマン関数の逆関数、実用上ほぼ$\mathcal{O}(1)$）
- groupCount/size/pathCount：$\mathcal{O}(\alpha(n))$
- groups：$\mathcal{O}(n)$
- 空間計算量：$\mathcal{O}(n)$
