...
- 時間計算量: $\mathcal{O}(V^3)$
- 距離行列と負閉路の影響判定を含む追加領域: $\mathcal{O}(V^2)$
- 入力辺の距離行列への反映: $\mathcal{O}(E)$

## バージョン情報

| バージョン番号     | 年月日     | 詳細                                                                                               |
|:-------------------|:-----------|:---------------------------------------------------------------------------------------------------|
| **バージョン 2.2** | 2026-08-23 | 初期化と入力辺の反映を1ループに統合し、負の自己ループを負閉路として維持                            |
| **バージョン 2.1** | 2026-08-23 | 内部配列参照と作業行の局所変数を整理。公開APIと計算量は変更なし                                    |
