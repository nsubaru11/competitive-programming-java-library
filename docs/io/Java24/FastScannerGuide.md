...
- Java 24版は`VarHandle`によるbyte配列アクセスを利用します。
...
| **バージョン 7.1** | 2026-08-10 | 行列・3 次元配列の `int` / `long` 向け 0-index・転置入力、`nextIntMultiset0` を追加。`nextIntMultiset` は 0-indexed の値域をそのまま読む仕様へ変更。                        |
