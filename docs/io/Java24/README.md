...
`FastIO17` の API をベースにしつつ、Java 24 で利用できる `VarHandle` ベースの低レベル最適化を取り入れ、配列・行列・コレクションの一括入出力と統合済み拡張
API を提供します。

最新更新日: `2026/03/12`

`2026/03/12` の最新更新では、入力側に `0-index` 補助メソッドが追加され、`next()` / `nextLine()` / `nextDouble()`
の処理が見直されました。出力側では既定バッファサイズが 1 MiB に統一され、容量不足時はいったんフラッシュし、1
回の書き込みが現バッファを超える場合のみ必要サイズへ拡張する実装になっています。
