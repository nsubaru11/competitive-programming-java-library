...
Manacherクラスは、Manacher'sアルゴリズムを実装して文字列内の回文を効率的に検出するためのクラスです。 このアルゴリズムは線形時間（$\mathcal{O}(n)$）で文字列内のすべての回文を見つけることができ、最長回文部分文字列の検索や任意の部分文字列が回文かどうかの判定に特に有用です。

## 特徴

- 線形時間（$\mathcal{O}(n)$）で全ての回文を検出する効率的なアルゴリズム
- 奇数長と偶数長の両方の回文に対応
- 最長回文部分文字列の取得
...
- 初期化（コンストラクタ）: $\mathcal{O}(n)$の時間計算量、$\mathcal{O}(n)$の空間計算量
- 長さ取得系メソッド: $\mathcal{O}(1)$の時間計算量
- 文字列取得系メソッド: $\mathcal{O}(1)$の時間計算量
- isPalindromeRange: $\mathcal{O}(1)$の時間計算量
