...
- 最大許容距離（k）を指定することで、bandedDPアルゴリズムによる高速化（$\mathcal{O}(\lvert S\rvert K)$の時間計算量）
...
| **バージョン 1.2** | 2025-07-22 | すべての最大許容距離（k）指定メソッドにbandedDPアルゴリズム（$\mathcal{O}(\lvert S\rvert K)$）を適用し高速化 |
