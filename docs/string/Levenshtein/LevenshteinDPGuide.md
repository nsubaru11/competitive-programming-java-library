...
LevenshteinDPクラスは、2つの文字列（または文字配列、整数配列）間のレーベンシュタイン距離（編集距離）を計算するためのユーティリティクラスです。 レーベンシュタイン距離とは、一方の文字列を他方に変換するために必要な最小の編集操作（挿入、削除、置換）の回数を表します。

## 特徴

- 複数のデータ型（String、char[]、int[]）に対応した編集距離計算メソッドを提供
- メモリ効率の良い実装（2次元配列ではなく、2つの1次元配列を使用）
- 最大許容距離（k）を指定することで、bandedDPアルゴリズムによる高速化（$\mathcal{O}(\lvert S\rvertK)$の時間計算量）
...
- 編集距離の計算は、文字列の長さに対して二次計算量（$\mathcal{O}(n*m)$）を持ちます
...
- 基本的な実装の時間計算量: $\mathcal{O}(n*m)$、ここでnとmは2つの入力配列の長さです
- 最大許容距離（k）を指定した場合の時間計算量: $\mathcal{O}(\lvert S\rvert K)$、ここで$\lvert S\rvert$は短い方の配列の長さ、Kは最大許容距離です
- 空間計算量:
	- 基本的な実装: $\mathcal{O}(\min(n,m))$、常に短い方の配列に対してのみメモリを確保します
	- bandedDP実装: $\mathcal{O}(K)$、Kは最大許容距離です
...
| **バージョン 1.2** | 2025-07-22 | すべての最大許容距離（k）指定メソッドにbandedDPアルゴリズム（$\mathcal{O}(\lvert S\rvertK)$）を適用し高速化 |
...
| 時間計算量 | $\mathcal{O}(\lvert S\rvert \times K)$ |
| メモリ     | $\mathcal{O}(\lvert S\rvert + K)$ ― 2 行ぶんをローリングバッファで保持 |
...
* **K が大きい場合** … 普通の $\mathcal{O}(\lvert S\rvert\lvert T\rvert)$ DP を使う方が速い場合もある。切替ロジックを用意。
...
| 時間計算量 | $\mathcal{O}(N \times D)$（$N = \lvert S\rvert + \lvert T\rvert$、$D$ = 編集距離） |
| メモリ     | $\mathcal{O}(N)$ ― 1 本の `int[] V` のみ                  |
