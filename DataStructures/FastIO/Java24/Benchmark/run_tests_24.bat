...
:: This script lives in "Data Structures\FastIO24\Benchmark\"
set "BENCH_PATH=%~dp0"
set "IO_PATH=%BENCH_PATH%.."
set "PROJECT_ROOT=%BENCH_PATH%..\..\..\"
set "SRC_PATH=%IO_PATH%\src"
...
%JAVA_EXE% %CP_FAST_START_OPTS% -Xlog:compilation*=debug,inlining*=debug:file="%JIT_LOG%":time,uptime,level,tags:filecount=0 -Xlog:gc*=debug,safepoint*=info:file="%DRY_RUN_GC_LOG%":time,uptime,level,tags:filecount=0 -cp "%BENCH_CLASSPATH%" Test1 %DRY_RUN_TOKEN_COUNT% %DRY_RUN_TOKEN_COUNT% %DRY_RUN_STRING_PRINT_COUNT% < "%INPUT_FILE%" > nul 2> "%DRY_RUN_STARTUP_LOG%"
if errorlevel 1 (
...
for /l %%i in (1, 1, %RUN_COUNT%) do (
    ...
    %JAVA_EXE% !T1_OPTS! -cp "%BENCH_CLASSPATH%" Test1 %TOKEN_COUNT% %TOKEN_COUNT% %STRING_PRINT_COUNT% < "%INPUT_FILE%" > "%TMP1%" 2> "!TEST1_STARTUP_LOG!"
    ...
    %JAVA_EXE% !T2_OPTS! -cp "%BENCH_CLASSPATH%" Test2 %TOKEN_COUNT% %TOKEN_COUNT% %STRING_PRINT_COUNT% < "%INPUT_FILE%" > "%TMP2%" 2> "!TEST2_STARTUP_LOG!"
