...
public class Example {
	...
	private static void demonstrateStringEditDistance() {
		...
		String s3 = "hello";
		String s4 = "hello";
		out.println("「" + s3 + "」と「" + s4 + "」の編集距離: " + Levenshtein.computeEditDistance(s3, s4));

		// 1文字違いの編集距離
		String s5 = "hello";
		String s6 = "hallo";
		out.println("「" + s5 + "」と「" + s6 + "」の編集距離: " + Levenshtein.computeEditDistance(s5, s6));
		...
		// 距離が許容範囲内の場合
		int maxDistance1 = 3;
		int result1 = Levenshtein.computeEditDistance(s1, s2, maxDistance1);
		out.println("「" + s1 + "」と「" + s2 + "」の編集距離 (最大許容距離=" + maxDistance1 + "): " + result1);

		// 距離が許容範囲を超える場合
		int maxDistance2 = 2;
		int result2 = Levenshtein.computeEditDistance(s1, s2, maxDistance2);
		...
		String s8 = "abcdefg";
		int maxDistance3 = 3;
		int result3 = Levenshtein.computeEditDistance(s7, s8, maxDistance3);
	...
	private static void demonstrateCharArrayEditDistance() {
		...
		char[] c1 = {'h', 'e', 'l', 'l', 'o'};
		char[] c2 = {'h', 'a', 'l', 'l', 'o'};
		int distance = Levenshtein.computeEditDistance(c1, c2);
		out.println("文字配列1と文字配列2の編集距離: " + distance);

		// 複数の編集操作が必要な場合
		char[] c3 = {'w', 'o', 'r', 'l', 'd'};
		char[] c4 = {'w', 'e', 'a', 'r'};
		out.println("文字配列3と文字配列4の編集距離: " + Levenshtein.computeEditDistance(c3, c4));
		...
		// 距離が許容範囲内の場合
		int maxDistance1 = 1;
		int result1 = Levenshtein.computeEditDistance(c1, c2, maxDistance1);
		out.println("文字配列1と文字配列2の編集距離 (最大許容距離=" + maxDistance1 + "): " + result1);

		// 距離が許容範囲を超える場合
		int maxDistance2 = 2;
		int result2 = Levenshtein.computeEditDistance(c3, c4, maxDistance2);
	...
	private static void demonstrateIntArrayEditDistance() {
		...
		int[] a1 = {1, 2, 3, 4, 5};
		int[] a2 = {1, 3, 4, 5, 6};
		int distance = Levenshtein.computeEditDistance(a1, a2);
		out.println("整数配列1と整数配列2の編集距離: " + distance);

		// 同一配列の編集距離
		int[] a3 = {10, 20, 30, 40};
		int[] a4 = {10, 20, 30, 40};
		out.println("整数配列3と整数配列4の編集距離: " + Levenshtein.computeEditDistance(a3, a4));

		// 複数の編集操作が必要な場合
		int[] a5 = {5, 10, 15, 20, 25};
		int[] a6 = {5, 15, 25, 35};
		out.println("整数配列5と整数配列6の編集距離: " + Levenshtein.computeEditDistance(a5, a6));
		...
		// 距離が許容範囲内の場合
		int maxDistance1 = 2;
		int result1 = Levenshtein.computeEditDistance(a1, a2, maxDistance1);
		out.println("整数配列1と整数配列2の編集距離 (最大許容距離=" + maxDistance1 + "): " + result1);

		// 距離が許容範囲を超える場合
		int maxDistance2 = 2;
		int result2 = Levenshtein.computeEditDistance(a5, a6, maxDistance2);
