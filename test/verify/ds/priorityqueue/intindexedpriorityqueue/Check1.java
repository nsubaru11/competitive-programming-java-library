package verify.ds.priorityqueue.intindexedpriorityqueue;
...
public final class Check1 {
	...
	private static void solve() {
		...
		int q = sc.nextInt();
		int[] s = sc.nextInt(n);
		IntIndexedPriorityQueue pq1 = new IntIndexedPriorityQueue(n + q);
		IntIndexedPriorityQueue pq2 = new IntIndexedPriorityQueue(n + q, true);
		for (int i = 0; i < n; i++) {
			pq1.set(i, s[i]);
			pq2.set(i, s[i]);
		}
		while (q-- > 0) {
			...
			if (t == 0) {
				...
				out.println(pq1.peek());
				pq2.remove(pq1.pollIndex());
			} else {
				out.println(pq2.peek());
				pq1.remove(pq2.pollIndex());
			}
		}
	}
